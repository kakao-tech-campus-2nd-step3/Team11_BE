...
### 4. 피해자 맞춤 AI 상담
**🏆 RAG 기반의 LLM 상담봇으로 피해자 사례에 맞는 맞춤 상담 제공**

- 📝 주요 기능 & 지표   
  - RAG 기반 LLM 상담봇 구축 (Recall 0.85, Faithfulness 0.9)
  - 의미 기반 청킹, 맥락 보존을 위한 제목을 내용에 포함하는 전략 사용
  - QNA와 용어 설명을 포함한 500개의 문서 청크로 벡터 DB 구축, KURE 기반 임베딩 사용
  - Similarity 검색 기반 Retriever 구성 및 Top-k 조회 최적화 (k=5)
  - End-to-End 기준 2초 이내 응답, P99 300ms 수준의 검색 지연 달성

- 🔍 트러블슈팅  
  **자동화 테스트 구축 :** RAG의 성능 테스트 수행과 기록에 시간이 소요되는 문제 발생

  - 해결 방안:
    - LangChain 을 이용한 테스트 자동화 파이프 라인 구축

  **검색 성능향상 :** RAG의 검색 성능이 Recall 0.3 이하로 떨어지는 문제 발생

  - 해결 방안:
    - 문서 청크에 맥락 정보를 가진 제목과 부제목이 자르면서, 의미 손실로 파악
    - 제목과 부제목을 내용에 포함하도록 청킹 로직 변경
    - 한국어에 친화적인 KURE 기반 임베딩 모델 사용
