...
- 회의 자료
  ...
  - [DB 운영 방식 결정](docs/discussion_notes/DB%20운영%20방식%20결정.md)
    ...
    > **📝 주요 내용**    
    >- H2 사용: 빠른 개발 속도와 스키마 유연성 장점, 환경 일관성 부재 단점    
    >- 개발/운영 DB 분리: 안전한 개발 환경 보장, 관리 부담 증가    
    >- 단일 DB 사용: 간편한 관리, 운영 데이터 위험 노출 가능성    
  ...
  - [Embedded 사용 방식](docs/discussion_notes/Embedded%20사용%20방식.md)
    ...
    > **📋 논의 안건**
    >- 모든 필드에 @Embedded 적용 vs 선택적 적용    
    >- 객체 생성 시점의 유효성 검증 전략    
    >- 도메인 모델의 복잡도와 유지보수성 간의 균형점 모색    
    
    > **📝 주요 내용**
    전체 필드 @Embedded 적용: 일관된 유효성 검사 가능, 과도한 복잡도 유발    
    >- 선택적 적용: 복잡한 객체(주소 등)에만 적용, 적절한 복잡도 유지    
    >- 유효성 검사는 API 요청 레벨에서 처리하여 도메인 로직 단순화    
  ...
  - [개발이 되지 않은 Entity를 사용하여 개발하여아 하는 경우 개발 방식 결정](docs/discussion_notes/개발이%20되지%20않은%20Entity를%20사용하여%20개발하여아%20하는%20경우%20개발%20방식%20결정.md)
    ...
    > **📝 주요 내용**
    >- 임시 엔티티 방식: 빠른 개발 가능, 코드 충돌 위험
    >- 단순 구조 방식: 안정적 개발, 개발 속도 저하
    >- 예상 도메인 방식: 빠른 개발, 리팩토링 부담
    >- 멘토링: 핵심 도메인 스키마 팀 정의 후 개발 권장
...
- 🔍 트러블 슈팅  
  **1. 진행도 타입을 구분 할 때 인터페이스와 전략클래스 비교:**
    

### 2. 서류 자동 완성 기능

**🏆 법률 서류 작성 부담을 줄이고, 크로스 플랫폼에서 정확하고 표준화된 PDF 문서를 제공하는 서비스**
...
### 3. 전세사기 예방 안전도 검사
**🏆 전세 계약 전 사전 위험을 파악하고 객관적인 안전성을 검증하며 추가 확인사항을 안내하는 서비스**

- 📝 주요 기능
  - 위험도 점수화 평가 식 적용 :`(집값*0.8 - 채권액 - 전세보증금 > 0)`

### 4. 지식인 채팅 상담
**🏆 실시간 전문가 상담으로 개인 맞춤 조언과 전문적인 법률 절차 안내를 제공하는 서비스**
...
- 🔍 트러블 슈팅  
  **1. 시간에 따른 상담 상태 자동 변경 문제**
      - 시간에 따른 STATUS값 변경 자동화를 위해 Spring Batch를 사용하려 했으나 서버가 부하 문제 발생 가능성 높음
  - 해결 방안:
      - 추가적인 의존성이 필요 없고 사용이 쉬운 Spring Scheduler를 사용
      - 매시간 정각마다 STATUS를 조회해 현재시간과 비교하여 업데이트

### 5. 피해자 커뮤니티 플랫폼
**🏆 피해자 간 정보 공유와 심리적 지지를 통해 안전한 소통 공간과 맞춤형 정보 접근성을 제공하는 서비스**
